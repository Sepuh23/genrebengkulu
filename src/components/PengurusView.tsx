...
// Definisikan ExtendedPengurus di sini juga
...
export function OrganizationManager({ pengurus, strukturJabatan, onUpdate }: OrganizationManagerProps) {
  ...
  const [editingItem, setEditingItem] = useState<ExtendedPengurus | StrukturJabatan | null>(null)
  ...
  // FIXED: Handle null/undefined with optional chaining
  const openModal = (item: ExtendedPengurus | StrukturJabatan | null = null) => {
    resetForms()
    if (item) {
      setEditingItem(item)
      if ('nama' in item) {
  ...
  // FIXED: Add validation for jabatan_id
